...
const CustomDropDown = (props: DropdownProps) => {
    const antMenu = <Menu style={{scrollPaddingRight: 20}}>
        {props.menu.map(item =>
            <Menu.Item
                key={item.key}
    ...
    return (
      <DropDownStyle overlay={antMenu} placement={props.placement}>
        <Button className="antd-dropdown--button">
          <CustomButton isDesktopText={props.isDesktopText} isMobileIcon={props.isMobileIcon}
                        type={props.type} variant={props.variant} size={props.size} text={props.text}
                        isDropDown icon={props.icon}/>
        </Button>
      </DropDownStyle>
    );
};
